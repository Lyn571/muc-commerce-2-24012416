...
def load_metric_api_data(base_dir: Path) -> list[dict]:
    ...
    return [_to_python_record(row) for row in data["metrics"]]
...
def load_category_api_data(base_dir: Path, selected_category: str = "全部") -> list[dict]:
    ...
    return [_to_python_record(row) for row in data["category_rows"]]


def _to_python_record(record: dict) -> dict:
    """把可能来自pandas/NumPy的标量转换为jsonify可处理的普通值。"""
    safe = {}
    for key, value in record.items():
        safe_value = value.item() if hasattr(value, "item") else value
